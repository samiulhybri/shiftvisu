...
class ItemStateDtoImport extends Command
{
    use DispatchesJobs;
     ...
     * @var string
     */
    protected $signature = 'import_dto:item_states';
    ...
    public function handle()
    {
        ...
        while ($chunk = $ds->itemStateDtos($skip, $take)) {
            $skip += $take;

            foreach ($chunk as $itemState) {
                ...
                $record->name = $itemState->name ?? $record->name;
                $record->is_active = $itemState->is_active ?? $record->is_active;
                $record->save();

                $record->halls()->detach();

                foreach ($itemState->hall_ids_custom as $hallCustomId) {
                    $hall = Hall::where(['custom_id' => $hallCustomId])->first();
                    if ($hall) $record->halls()->attach($hall->id);
                }
            }
        }
