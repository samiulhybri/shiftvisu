...
class ClassificationDtoImport extends Command
{
    use DispatchesJobs;
    /**
     ...
     * @var string
     */
    protected $signature = 'import_dto:classification';
    ...
    public function handle(): int
    {
        ...
        while ($chunk = $ds->classificationDtos($skip, $take)) {
            $skip += $take;
            foreach ($chunk as $classification) {
                ...
                if ($classification->model_type === Item::class && $items->has($classification->model_id_custom)) {
                    Classification::updateOrCreate(
                        [
                            'model_type' => $classification->model_type,
                            'model_id' => $items[$classification->model_id_custom],
