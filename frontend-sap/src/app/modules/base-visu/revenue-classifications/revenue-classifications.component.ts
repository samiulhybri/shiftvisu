...
@Component({
	selector: "app-revenue-classifications",
	templateUrl: "./revenue-classifications.component.html",
	styleUrl: "./revenue-classifications.component.css",
})
export class RevenueClassificationsComponent implements OnInit {
	...
	selectedRevenueClassifications: RevenueClassification = new RevenueClassification().deserialize(
		{}
	);
	@ViewChild("errorDialogRevenueClassifications", { static: false })
	errorDialogRevenueClassifications: any;
	...
	newButtonClick() {
		...
		this.selectedRevenueClassifications = new RevenueClassification().deserialize({});
		this.selectedRevenueClassifications.custom_id = this.customId;
		if (!this.customId) {
			this.selectedRevenueClassifications.custom_id = "";
			this.getCustomId();
		}
	}
	...
	editClick(value: object): void {
		...
		this.customIdState = "None";
		this.selectedRevenueClassifications =
			this.selectedRevenueClassifications?.deserialize(value);
		this.cachedCustomId = this.selectedRevenueClassifications.custom_id;
		this.disableButtonDuringRequest = false;
	}
	...
	onSubmit(form: NgForm) {
		...
		const customId = this.selectedRevenueClassifications.custom_id?.trim();
		this.selectedRevenueClassifications.custom_id = customId;
		if (this.isUpdate) {
	...
	async onCreateOrUpdate() {
		this.isLoading = true;
		const payload = this.selectedRevenueClassifications?.toOdata();
		const method = this.isUpdate ? "put" : "post";
		const urlString = this.isUpdate
			? `RevenueClassifications(${this.selectedRevenueClassifications?.id})`
		...
		this.commonService[method](urlString, payload).subscribe({
			...
			error: () => {
				this.disableButtonDuringRequest = false;
				this.isLoading = false;
				this.errorDialogRevenueClassifications.elementRef.nativeElement.open = true;
			},
		});
	}
	...
	checkCustomId() {
		...
		const urlString = `RevenueClassifications?$filter=custom_id eq '${this.selectedRevenueClassifications.custom_id}'&$select=custom_id`;
		if (result.success) {
	...
	async getCustomId() {
		...
		try {
			this.customId = await this.commonService.getEntity("RevenueClassifications");
			if (this.customId) {
				this.selectedRevenueClassifications.custom_id = this.customId;
			} else {
				this.selectedRevenueClassifications.custom_id = "";
	...
	deleteSubmit() {
		...
		this.disableButtonDuringRequest = true;
		this.isLoading = true;
		this.commonService.delete(`/RevenueClassifications(${this.selectedId})`).subscribe({
			next: () => {
	...
	closeErrorDialog() {
		this.errorDialogRevenueClassifications.elementRef.nativeElement.open = false;
	}
}
