...
import Crucibles from "@app/shared/models/crucible.model";
...
@Component({
	selector: "app-crucibles",
	templateUrl: "./crucibles.component.html",
	styleUrl: "./crucibles.component.css",
})
export class CruciblesComponent {
	isUpdateDialog = false;
	selectedRowValue = new Crucibles().deserialize({});
	value!: string;
	...
	constructor(
		...
		this.cruciblesConfig = this.configService.getConfigValue("crucibles");
	}
	...
	deleteClick(value: any): void {
		this.selectedRowValue = new Crucibles().deserialize(value);
	...
	editClick(value: object): void {
		...
		this.selectedRowValue = new Crucibles().deserialize(value);
		(document.getElementById("cruciblesDialog") as Dialog).open = true;
	...
	async newButtonClick() {
		...
		this.selectedRowValue = new Crucibles().deserialize({});
		this.customIdState = "None";
	...
	async getCustomId() {
		...
		this.customId = await this.commonService.getEntity("Crucibles").catch(() => false);
		this.selectedRowValue.custom_id = this.customId;
	...
	refreshEditData() {
		const url = `Crucibles?$filter=id eq ${this.selectedRowValue?.id}`;
	...
	deleteSubmit() {
		...
		this.isLoading = true;
		this.commonService.delete(`/Crucibles(${this.selectedRowValue.id})`).subscribe({
			next: () => {
				this.disableButtonDuringRequest = false;
	...
	checkCustomId() {
		...
		const urlString = `Crucibles?$filter=custom_id eq '${this.selectedRowValue.custom_id}'&$select=custom_id`;
		if (result.success) {
	...
	async onCreateOrUpdate() {
		...
		const urlString = this.isUpdateDialog
			? `Crucibles(${this.selectedRowValue?.id})`
			: `Crucibles`;
	...
	shouldBeDisabled(fieldName: string) {
		if (this.cruciblesConfig && this.selectedRowValue) {
			return (
				this.cruciblesConfig[fieldName] === 0 && this.selectedRowValue.is_imported_from_erp
			);
		} else {
			return false;
		}
