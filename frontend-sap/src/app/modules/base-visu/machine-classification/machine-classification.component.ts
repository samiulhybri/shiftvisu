...
@Component({
	selector: "app-machine-classification",
	templateUrl: "./machine-classification.component.html",
	styleUrl: "./machine-classification.component.css",
})
export class MachineClassificationComponent implements OnInit {
	...
	selectedMachineClassifications: MachineClassification = new MachineClassification().deserialize(
		{}
	);
	@ViewChild("errorDialogMachineClassifications", { static: false })
	errorDialogMachineClassifications: any;
	...
	newButtonClick() {
		...
		this.selectedMachineClassifications = new MachineClassification().deserialize({});
		this.selectedMachineClassifications.custom_id = this.customId;
		if (!this.customId) {
			this.selectedMachineClassifications.custom_id = "";
			this.getCustomId();
		}
	}
	...
	editClick(value: object): void {
		...
		this.customIdState = "None";
		this.selectedMachineClassifications =
			this.selectedMachineClassifications?.deserialize(value);
		this.cachedCustomId = this.selectedMachineClassifications.custom_id;
		this.disableButtonDuringRequest = false;
	}
	...
	onSubmit(form: NgForm) {
		...
		const customId = this.selectedMachineClassifications.custom_id?.trim();
		this.selectedMachineClassifications.custom_id = customId;
		if (this.isUpdate) {
	...
	async onCreateOrUpdate() {
		this.isLoading = true;
		const payload = this.selectedMachineClassifications?.toOdata();
		const method = this.isUpdate ? "put" : "post";
		const urlString = this.isUpdate
			? `MachineClassifications(${this.selectedMachineClassifications?.id})`
		...
		this.commonService[method](urlString, payload).subscribe({
			...
			error: () => {
				this.disableButtonDuringRequest = false;
				this.isLoading = false;
				this.errorDialogMachineClassifications.elementRef.nativeElement.open = true;
			},
		});
	}
	...
	checkCustomId() {
		...
		const urlString = `MachineClassifications?$filter=custom_id eq '${this.selectedMachineClassifications.custom_id}'&$select=custom_id`;
		if (result.success) {
	...
	async getCustomId() {
		...
		try {
			this.customId = await this.commonService.getEntity("MachineClassification");
			if (this.customId) {
				this.selectedMachineClassifications.custom_id = this.customId;
			} else {
				this.selectedMachineClassifications.custom_id = "";
	...
	deleteSubmit() {
		...
		this.disableButtonDuringRequest = true;
		this.isLoading = true;
		this.commonService.delete(`MachineClassifications(${this.selectedId})`).subscribe({
			next: () => {
	...
	closeErrorDialog() {
		this.errorDialogMachineClassifications.elementRef.nativeElement.open = false;
	}
}
